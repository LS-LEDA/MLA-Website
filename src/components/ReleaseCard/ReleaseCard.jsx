...
const ReleaseCard = (props) => {
  ...
  return (
    <motion.li
      className="flex w-full h-auto rounded-md border border-transparent hover:border-primary hover:dark:border-dark_primary
    ...
    >
      <a
        href={props.platform.browser_download_url}
      ...
      >
        <BsBox
          style={
            isHovering
              ? { filter: 'drop-shadow(0px 1px 2px rgb(6 182 212))' }
              : {}
          }
        />
        <span className="flex-1">{props.platform.name}</span>
        <span className="hidden sm:flex w-32 text-gray-500">
          {formatBytes(props.platform.size)}
        </span>
        <span className="hidden md:flex w-24 text-gray-500">
          {releaseDate.toDateString().split(' ').slice(1).join(' ')}
        </span>
        {isHovering && (
          <motion.div
            initial="hidden"
