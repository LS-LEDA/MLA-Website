...
const Release = (props) => {
  ...
  return (
    <div className="flex flex-col w-full py-10 space-y-5">
      ...
      <div
        className={`${
          rsNotes ? 'flex-col' : 'hidden'
        } fixed flex translate-x-[-50%] translate-y-[-50%] top-[50%] left-[50%] max-w-7xl max-h-96 bg-dark_bg text-dark_text dark:bg-bg dark:text-text rounded-md p-5 z-50`}
      >
        ...
        <ReactMarkdown
          className={`flex-col mt-2 w-full h-full overflow-y-auto space-y-2 ${styles.reactMarkDown}`}
        >
          {props.release.body}
        </ReactMarkdown>
      </div>
